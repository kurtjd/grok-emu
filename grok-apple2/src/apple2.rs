...
use grok_6502::{Cpu, bus::Bus, bus::SimpleBus};

use std::path::Path;
use std::{fs::File, io::Read};

use sdl2::{Sdl, render::Canvas, render::TextureCreator, video::Window, video::WindowContext};
...
mod soft_switch {
    pub const INPUT_CLEAR: u16 = 0xC010; // Whole page
}
...
impl<'a> Apple2<'a> {
    fn load_rom(&mut self) {
        ...
        fw_rom
            .read_exact(&mut self.mem_manager.memory[address::FW_START..])
    ...
    fn handle_soft_sw(&mut self) {
        match self.bus.addr() {
            soft_switch::INPUT_CLEAR => self.mem_manager.memory[address::INPUT_DATA] &= !(1 << 7),
            0xC030..0xC050 => self.snd_handler.handle_soft_sw(self.bus.addr() as usize),
            0xC050..0xC080 => self.gfx_handler.handle_soft_sw(self.bus.addr() as usize),
            0xC080..0xC090 => self
                .mem_manager
                .handle_soft_sw(self.bus.addr(), self.bus.op()),
            0xC090..=0xC0FF => self
                .disk_controller
                .handle_soft_sw(self.bus.addr() as usize, &mut self.mem_manager.memory),
            _ => (),
        }
    }
    ...
    pub fn run_frame(&mut self, frame_rate: u32) {
        ...
        while frame_cycles < cycles_per_frame {
            ...
            self.mem_manager.tick(&mut self.bus);
            self.handle_soft_sw();
            self.bus.tick();
            self.cpu.tick(&mut self.bus);
    ...
    pub fn draw_frame(&mut self, frame_rate: u32) {
        self.gfx_handler
            .handle_gfx(frame_rate, &self.mem_manager.memory);
    }

    pub fn input_char(&mut self, ascii: u8) {
        self.mem_manager.memory[address::INPUT_DATA] = ascii;
    }
