...
impl<B: BusHandlerZ80> Cpu<B> {
    ...
    pub(crate) fn fetch_t3(&mut self, bus: &mut B) {
        self.reg.ir = bus.data();
    ...
    pub(crate) fn mem_wr_t1(&mut self, bus: &mut B, addr: u16, data: u8) {
        bus.set_addr(addr);
        bus.set_data(data);
        bus.set_mreq(true);
        bus.set_wr(true);
    ...
    pub(crate) fn mem_wr_t2(&mut self, bus: &mut B) {
        bus.set_mreq(false);
        bus.set_wr(false);
        self.check_wait(bus);
    }

    pub(crate) fn mem_wr_t3(&mut self, _bus: &mut B) {}
    ...
    pub(crate) fn io_rd_t2(&mut self, bus: &mut B) {
        bus.set_iorq(true);
        bus.set_rd(true)
    }
    ...
    pub(crate) fn io_wr_t1(&mut self, bus: &mut B, port: u8, data: u8) {
        let addr = u16::from_be_bytes([self.reg.gpr.a, port]);
        bus.set_addr(addr);
        bus.set_data(data);
    }

    pub(crate) fn io_wr_t2(&mut self, bus: &mut B) {
        bus.set_iorq(true);
        bus.set_wr(true);
    }

    pub(crate) fn io_wr_t3(&mut self, bus: &mut B) {
        bus.set_iorq(false);
        bus.set_wr(false);
        self.check_wait(bus);
    }

    pub(crate) fn io_wr_t4(&mut self, _bus: &mut B) {}
